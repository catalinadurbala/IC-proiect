...
def protocol(c, s, p):
    s.homegabar = HomomorphicVec(dim=12, bitlen=23, signed=True, passive=True)
    s.hgamma = HomomorphicVec(dim=52, bitlen=8, signed=False, passive=True)
    ...
    c.gamma = UnsignedVec(bitlen=8, dim=52, empty=True, signed=False).input(src=driver, desc='gamma')
    conversions.UnsignedVec_PaillierVec_send(c.gamma, s.hgamma, 8, [52], False)
