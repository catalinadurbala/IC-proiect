...
def protocol(c, s, p):
    s.homegabar = HomomorphicVec(dim=12, bitlen=23, signed=True)
    s.hgamma = HomomorphicVec(dim=52, bitlen=8, signed=False)
    ...
    s.psi = UnsignedVec(bitlen=8, dim=52, empty=True, signed=False).input(src=driver, desc='psi')
    s.u = SignedVec(bitlen=8, dim=(12, 52), empty=True, signed=True).input(src=driver, desc='u')
    s.tau = Unsigned(bitlen=50, empty=True, signed=False, dim=[1]).input(src=driver, desc='tau')
    conversions.UnsignedVec_PaillierVec_receive(c.gamma, s.hgamma, 8, [52], False)

    for i in xrange(12):
        s.homegabar[i] = Homomorphic(val=-s.u[i].dot(s.psi), signed=True, bitlen=22, dim=[1]) + s.hgamma.dot(s.u[i])
